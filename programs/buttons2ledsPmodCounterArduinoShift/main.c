...
#define ARDUINO     *((volatile uint32_t *) 0x0001000c)
...
int main() {
    ...
    for (;;) {
        uart_puts("Press buttons, should turn on leds, while counter goes up in pmod0 and the arduino connector shifts!\r\n");
        LEDS = ~BUTTONS;
        PMOD0 = counter++;
